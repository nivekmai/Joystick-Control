...
if check_os('win') and is_64_bit():
    with SDL2_WIN64.as_file() as sdl_dll:
        os.environ.setdefault('PYSDL2_DLL_PATH', os.path.join(os.path.dirname(os.path.realpath(__file__)), "sdl2_win64"))
        from .. import sdl2
elif check_os('win'):
...
def joystick_key_from_event(event: sdl2.SDL_Event, joystick: Joystick|None=None):
...
def run_event_loop(
    add_joystick: Callable[[Joystick], None],
    remove_joystick: Callable[[Joystick], None],
    handle_key_event: Callable[[Key], None],
    alive: Callable[[], bool] | None = None,
    key_from_event: Callable[[sdl2.SDL_Event], Key] | None = None,
    **kwargs
):
